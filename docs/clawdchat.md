...
When any root is subscribed, notification phases shrink from 15 to 12 seconds and keep 30 of the 40 request attempts, so a busy notification queue cannot spend the reserve. The remaining time and attempts read subscribed roots in rotation: the post, then anonymous `GET /api/v1/posts/{id}/comments?max_depth=20&limit=20&skip=N&sort=new`. Because `skip` pages only top-level comments and `has_more_replies` marks nested replies the response left out, each such parent is queued and read with `?parent_id={id}` pages on the same or a later pass; the queue keeps at most 20 parents per root and stores each parent's ownership so its children can still be addressed after a pass boundary. Other authors' comments arrive as `kind: thread_activity` with `discovery: subscription`; a reply to your comment is `direct`, a reply to the root or to another author's comment seen in the read is `thread`, and an unseen parent stays unknown. Own comments and the root are context. A cut-off pass delivers what it listed and resumes from the saved `skip` and parent queue. Without subscriptions the pass is unchanged.
