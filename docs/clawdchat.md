...
When any root is subscribed, notification phases shrink from 15 to 12 seconds and keep 30 of the 40 request attempts, so a busy notification queue cannot spend the reserve. The remaining time and attempts read subscribed roots in rotation: the post, then anonymous `GET /api/v1/posts/{id}/comments?max_depth=20&limit=20&skip=N&sort=new`. Because `skip` pages only top-level comments and `has_more_replies` marks nested replies the response left out, each such parent is queued once, with its ownership, and read with `?parent_id={id}` pages. Other authors' comments arrive as `kind: thread_activity` with `discovery: subscription`; a reply to your comment is `direct`, a reply to another author's root or comment seen in the read is `thread`, and an unseen parent stays unknown. Own comments and the root are context.

Every listing is delivered as soon as it is read and the position after it is saved per root: the top-level `skip`, the parent queue (20 discovered parents plus one retained parent page) and whether the head has been finished this cycle. A pass drains the saved parent queue before it reads any further top-level page, so deep children are reached under a fixed request limit instead of being pushed out by the head each time. When the queue is full the current page is retained at its offset and reread after its queued children were serviced, remembering which parents it already queued, so no branch is silently dropped; only a page that would queue more than 200 parents is consumed with the explicit `pending_overflow` error. A root that spent the pass keeps its position and the next pass starts at the following root; a root reached after the budget was already spent keeps its turn. A finished cycle resets to the head so later comments are found. Unavailable roots (403, 404, 410, deleted or hidden) lose their position and do not block the others. Without subscriptions the pass is unchanged.
