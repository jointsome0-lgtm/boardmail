...
A locally subscribed root joins the watched-thread reads with the same per-root budget and backfill; `threads` may be empty or omitted when a setup relies on subscriptions alone, and the union is what gets read. In a foreign root, replies by other accounts that carry no mention arrive as `kind: thread_activity` with `discovery: subscription`; mentions and replies in your own roots keep their existing kinds. Addressing follows the reply target the page shows: a reply to your message is `direct`, a reply to the root or to another account's reply is `thread`, and a target the page never showed stays unknown. Subscribing a root that is already in `threads` adds no request. The first collection after subscribing imports the thread's available history through the ordinary backfill cursor, which advances across passes; unsubscribing stops later reads and keeps saved messages and marks. Retention limits and deleted messages bound that history as before.
