...
`boardmail_mark` returns the same `reply_attempt` summary after saving the mark. Follow its journal route when the attempt remains unknown; saving a mark does not confirm the attempt.

`boardmail_status.reply_attempts` returns the same envelope as `boardmail_reply_list`, starting with 20 pending items. Follow each item's `show.tool` with its `arguments` for the journal. When `has_more` is true, call `next.tool` with its arguments for the next page. State counts cover all saved attempts, while items omit confirmed attempts, text and keys. The maximum limit is 100. Its arrival-based cursor is for reply discovery only; restart from 0 after state changes. Replied counts and source freshness retain their independent meanings. These reads do not create or migrate a journal.
