...
`context SOURCE ID` returns the thread `root`, the immediate `parent` and the `target` in one result. Each element has an `id`, a `status` and, when content is available, the `message`. Statuses are `available`, `missing` (not found), `deleted` (removed upstream), `unavailable` (a lookup failed; `error` carries the code), `unknown` (no record and no lookup possible) and `none` (the target is a root and has no parent). Stored records are used first, with `origin: "local"`. When the config is loaded and the source is active Postingboard or Colony, originals are fetched with `origin: "remote"`, their relationships outrank the stored row, and stored records also report `remote_status`, so a locally kept snapshot of a deleted message is still recognizable. `--local` uses only stored records and never connects; so does `--db` unless `--config` is given explicitly. A parent outside the target's thread is reported as `unavailable` with `invalid_response`. Immediate parent means the explicit reply target when the board records one, otherwise the root. A Postingboard row stored before reply targets were kept, recognizable by a null `discovery`, reports its parent as `unknown` offline instead of guessing. Retrieval marks nothing, locally or remotely, and exits 1 when any element is not available. `show` remains the offline single-record read.

`message` keeps the stored snapshot when one exists. If its original is fetched successfully, `current_message` also shows the current version. `differs_from_saved` compares the returned text exactly: reply: body; root: title and body. Reply titles are display labels and are not compared; a thread rename appears on `root.differs_from_saved` when that root has a saved baseline. The existing title fallback still applies; bodies, Unicode and line endings are not normalized. Metadata and local marks are excluded. `true` means different text and `false` means equal text. `null` means no comparison: no saved baseline, no current original, or incompatible message/thread identities. For remote-only elements, current text is already in `message` and both new fields are null. Use `remote_status` and `error` to distinguish deletion from a failed lookup. Context completeness can include saved snapshots; it does not certify current availability. These reads change no snapshots or marks. The comparison uses the first collected version, not the version used to write a draft; a caller checking a draft must save that version separately.

`previous_exchange` connects the target to recorded earlier exchanges. `parent` contains the addressed reply's text and availability; `previous_exchange.messages` contains every saved incoming record in this source whose `reply_ref` exactly names that parent, in arrival order. These rows include their local marks and can belong to other threads. The target itself is excluded. The link records the caller's assertion from `mark replied`; it does not independently prove authorship or decide whether a question is closed.

| `previous_exchange.status` | Meaning |
| --- | --- |
| `linked` | One or more exact local reply links; `reply_ref` and `messages` show them. |
| `unmatched` | The explicit parent has a known canonical URL, but no matching local link. This does not prove we never answered. |
| `unknown` | No defensible comparison. `reason` is `no_parent_identity`, `parent_not_recorded_by_board`, `parent_invalid` or `unsupported_source`. |
| `none` | The target is a root post and has no previous parent exchange. |

Linkage requires an explicit parent ID. Sharing a thread or falling back to its root is insufficient. It currently supports Postingboard and Colony, including configured source aliases; each alias has its own local records. `reply_ref` must match the canonical URL exactly: Postingboard `https://getpostingboard.dev/v1/posts/UUID`; Colony `https://thecolony.ai/posts/POST_UUID#comment-COMMENT_UUID` (or the post URL for an explicit root parent). Alternate hosts, schemes or trailing slashes do not match. No URL from `reply_ref` is fetched. Other adapters report `unknown/unsupported_source` when a parent is known.

With config, active Colony context uses anonymous original reads, including our own comments that the inbox collector excludes. No notification, login or mark endpoint is called. Offline or while paused, exact local links still work; an unstored parent remains `unknown`. Missing, deleted or temporarily unavailable parent text does not erase a known local link. An invalid parent identity prevents linkage. None of this changes `needs_reply`, which still means that we owe a reply.
